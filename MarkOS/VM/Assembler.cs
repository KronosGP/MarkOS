...
namespace MarkOS.VM
{
    public class Assembler
    {
        ...
        public static MenadzerKomend menadzer= new MenadzerKomend();

        public static bool AssembleFile(string fileIn, string fileOut)
        {
            try
            {
                if(menadzer.Wejscie("plik ch "+fileIn)=="tak")
                {
                    string[] data = menadzer.Wejscie("plik r " + fileIn).Split("\n");
                    Input = DataUtilis.StringArrayToList(data);

                    StoreVariables();
                    if (!StoreLabels()) { Console.WriteLine("[ERROR] ", ConsoleColor.Red); Console.WriteLine("Problem occured while storing labels");return false;}
                    if (!ReplaceLabels()) { Console.WriteLine("[ERROR] ", ConsoleColor.Red); Console.WriteLine("Problem occured while replacing labels with jumps."); return false; }
                    if (!AssembleCode(fileOut)) { Console.WriteLine("[ERROR] ", ConsoleColor.Red); Console.WriteLine("Problem occured while converting assmebler to bytecode."); return false; }
                    return true;
                }
                else
                {
                    Console.WriteLine("File system error!", ConsoleColor.Red);
                    Console.WriteLine("Input: " + fileIn, ConsoleColor.White);
                    Console.WriteLine("Output: " + fileOut, ConsoleColor.White);
                    return false;
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("Unknown error has occured while assembling!", ConsoleColor.Red);
                Console.Write("[INTERNAL] ", ConsoleColor.Red); Console.WriteLine(ex.Message, ConsoleColor.White);
            }
            return false;
        }
