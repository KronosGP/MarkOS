...
namespace MarkOS.VM
{
    public static class CPU
    {
        ...
        public static string TitleBarText = "Mark OS";
        ...
        public static void Clock()
        {
            if (!IsHalted)
            {
                ...
                if (ProgCtr >= Memory.Size)
                {
                    ...
                    DrawFresh();
                    ProgCtr = 0;
                    Console.WriteLine("Program counter left memory space", ConsoleColor.DarkYellow);
                    Console.ReadLine();
                }
        ...
        public static void DrawDebug()
        {
            // draw registers
            string regs = "";
            for (int i = 0; i < MaxRegisters; i++) { regs += DataUtilis.IntToHex(Registers[i], "X2") + " "; }
            if (regs.Length > 0) { TextGraphics.DrawString(0, CLI.Height - 1, regs, Color.Cyan, CLI.BackColor); }

            // draw program counter
            TextGraphics.DrawString(48, CLI.Height - 1, "PC=" + DataUtilis.IntToHex(ProgCtr), Color.Yellow, CLI.BackColor);

            // draw i register
            TextGraphics.DrawString(58, CLI.Height - 1, "I=" + DataUtilis.IntToHex(I), Color.Green, CLI.BackColor);

            // draw w register
            TextGraphics.DrawString(67, CLI.Height - 1, "W=" + DataUtilis.IntToHex(W), Color.Magenta, CLI.BackColor);
